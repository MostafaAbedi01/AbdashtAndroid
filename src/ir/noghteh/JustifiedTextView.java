...
public class JustifiedTextView extends View {
	...
	/**
	 * when we want to draw text after view created to avoid loop in drawing we
	 * use this boolean
	 */
	...
	private void constructor(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			// setText(XmlToClassAttribHandler.GetAttributeStringValue(mContext,
			// attrs, namespace, key, ""));
	...
	/***
	 * this method get the string and divide it to a list of StringLines
	 * according to textAreaWidth
	 * 
	 * @param originalText
	 * @return
	 */
	private List<String> divideOriginalTextToStringLineList(String originalText) {
		...
		for (int j = 0; j < listParageraphes.length; j++) {
			...
			for (int i = 0; i < arrayWords.length; i++) {
				...
				// if text width is equal to textAreaWidth then just add it to
				// ListStringLine
				...
				// else if text width excite textAreaWidth then remove last word
				// and justify the StringLine
				else if (getTextAreaWidth() < textWidth) {

					int lastWordCount = arrayWords[i].length();

					// remove last word that cause line width to excite
					// textAreaWidth
	...
	/**
	 * this method add space in line until line width become equal to
	 * textAreaWidth
	 * 
	 * @param lineString
	 * @param lineWidth
	 ...
	 * @return
	 */
	private String justifyTextLine(TextPaint textPaint, String lineString,
			int textAreaWidth) {
		...
		while (lineWidth < textAreaWidth && lineWidth > 0) {
			...
			lineString = lineString.substring(0, gapIndex) + "  "
					+ lineString.substring(gapIndex + 1, lineString.length());
	...
	/***
	 * this method calculate height for a line of text according to defined
	 * TextPaint
	 * 
	 * @param textPaint
	 */
	...
	/***
	 * this method calculate view's height according to line count and line
	 * height and view's width
	 * 
	 * @param lineListSize
	 * @param lineHeigth
	 * @param lineSpace
	 */
	public void setMeasuredDimentions(int lineListSize, int lineHeigth,
			int lineSpace) {
	...
	/***
	 * Sets the string value of the JustifiedTextView. JustifiedTextView does
	 * not accept HTML-like formatting. Related XML Attributes -noghteh:text
	 * 
	 * @param text
	 */
	...
	public void setTextSize(int unit, float textSize) {
		textSize = TypedValue.applyDimension(unit, textSize, mContext
